...
export function SidebarItem({
  ...
  return (
    <Link
      ...
      prefetch={prefetch}
      className={cn(
        "flex flex-row items-center gap-2 px-4 py-2.5 transition-colors duration-200 border-l-2 border-transparent",
...
export function SidebarFolderTrigger({
  ...
  if (collapsible) {
    return (
      <CollapsibleTrigger
        className={cn(
          "flex flex-row items-center w-full gap-2 px-4 py-3 transition-colors duration-200",
          "font-mono text-[9px] uppercase tracking-[0.2em] text-white/30 hover:text-white/60",
          props.className,
        )}
        {...props}
      >
        ...
        <ChevronDown
          data-icon
          className={cn(
            "ms-auto size-3 transition-transform duration-300",
...
export function SidebarFolderLink({
  ...
  return (
    <Link
      ...
      prefetch={prefetch}
      className={cn(
        "flex flex-row items-center w-full gap-2 px-4 py-2.5 transition-colors duration-200 border-l-2",
        "font-mono text-[9px] uppercase tracking-[0.2em]",
        active
          ? "bg-white/5 text-white border-white"
...
export function Sidebar({ children }: { children: ReactNode }) {
  return (
    <SidebarProvider>
      <SidebarContent>
        {({ ref, hovered, onPointerEnter, onPointerLeave }) => (
          <aside
            ref={ref}
            id="nd-sidebar"
            ...
            className={cn(
              "fixed top-fd-nav h-fd-main w-fd-sidebar min-w-0 max-md:hidden z-30 transition-all bg-[#020202] border-r border-white/5",
              hovered && "w-[var(--fd-sidebar-width)]",
            )}
          >
            <SidebarViewport>
